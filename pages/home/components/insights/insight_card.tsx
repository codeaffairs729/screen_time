...
const InsightCard = ({
    ...
    return (
        <div
            className={clsx(
                `flex flex-col h-[100px] w-[100px] sm:h-[227px]  sm:w-[305px] md:shadow-custom-5 shadow-md md:rounded-[10px] rounded-[2px] `,
                className
            )}
        >
            <div className=" flex flex-row  sm:px-4 justify-center items-center sm:p-2 p-1">
                <div className=" md:text-[22px] sm:text-sm text-xs text-center ">
                    {" "}
                    {label}{" "}
                </div>
                <InfoIcon
            ...
            <div className="flex flex-col lg:flex-row justify-center items-center 2xl:px-16 lg:py-8 space-y-2 lg:space-x-4 lg:px-4">
                <div className=" md:w-[60%]">
                    {getImage(label, insightIcon, isMobile, metrics)}
                </div>
...
const getImage = (
    ...
    if (label == "Datasets")
        return (
            <div>
                <img src={insightIcon} width={isMobile ? 18 : 191} />
            </div>
        );
    else if (label == "Data Providers" && isMobile)
        return (
            <div>
                {/* <img
                    src={insightIcon}
                    width={isMobile ? 25 : 120}
                    className="xl:ml-4 2xl:ml-1"
                /> */}
            </div>
        );
    else if (insightIcon.length > 0) {
        return (
            <div>
                <img
                    src={insightIcon}
