...
const LearnMore = ({ isMobile }: { isMobile: boolean }) => {
    ...
    return (
        <div>
            <div
                className={clsx(
                    `w-full py-3 sm:py-16 md:text-3xl text-[19px] overflow-hidden cursor-pointer text-dtech-new-main-light font-bold   ${
                        learnMore ? "!h-full" : "mb-0"
                    } }`
                )}
                style={
                    !isMobile
                        ? {
                              backgroundImage: `url(images/bghome.svg)`,
            ...
            >
                ...
                >
                    ...
                    <div className=" w-fit shadow-custom-1 bg-dtech-new-main-light active:bg-dtech-dark-yellow hover:bg-dtech-main-dark active:border-b-2 border-black hover:border-0 active:text-black text-white rounded-full p-2 mt-5 hover:rounded-full focus-within:rounded-full focus:rounded-full focus-visible:rounded-full active:rounded-full focus-within:bg-[#FDD522] focus-within:border-b-2 focus-within:border-black focus:bg-[#FDD522] animate-bounce">
                        {!learnMore ? (
            ...
            >
                <div
                    className={!learnMore ? " hidden" : "block md:mb-20 mb-10"}
                >
                    <div className="flex flex-row px-6 sm:px-[10%] sm:items-center justify-between sm:justify-start">
                        <div className=" flex flex-row py-10">
                            <div className="font-[700] sm:text-xl text-md md:text-[30px] text-dtech-new-main-light md:text-[#333333]">
                                Helping Data Users & Providers
                            </div>
                        </div>
                    </div>
                    {helpData.map((item, index) => {
                        return (
                            item.label !== "DATA ENABLERS" && (
                                <HelpComponent
                                    key={index}
                                    item={item}
                                    index={index}
