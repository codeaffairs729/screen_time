...
const Row = ({ dataset }: { dataset: Dataset }) => {
  ...
  return (
    <>
      ...
      <Cell className="text-center">{<StarRating rating={dataset.detail.popularity} />}</Cell>
      <Cell className="text-center">{<StarRating rating={dataset.detail.dataQuality} />}</Cell>
      <Cell className="text-xs text-center pt-1 font-medium text-gray-700">{dataset.detail.lastUpdate.toLocaleString({...DateTime.DATE_FULL })}</Cell>
      <Cell>{dataset.detail.lastDownloaded}</Cell>
      <Cell className="text-center pt-1"><Favourite isFavourited={dataset.detail.isFavourited}/></Cell>
    </>
  );
};
